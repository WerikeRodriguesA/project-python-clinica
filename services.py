...
def verificar_conflito(medico_nome, data_hora_str):
    """
    Verifica se já existe uma consulta agendada para o mesmo médico
    no mesmo horário. Retorna True se houver conflito.
    """
    for consulta in consultas:
        if consulta["medico_nome"] == medico_nome:
            if consulta["data_hora"] == data_hora_str:
...
def registrar_falta(consulta_id):
    ...
    Retorna True se registrou, False se não encontrou.
    """
    for consulta in consultas:
        if consulta["id"] == consulta_id:
            if consulta["status"] != "agendada":
            ...
            consulta["valor_total"] = consulta["valor_total"] + MULTA_FALTA
...
def criar_prontuario(consulta_id, procedimentos, observacoes):
    """
    Cria um prontuário vinculado a uma consulta concluída.
    procedimentos é uma lista de dicionários: [{"nome": "limpeza", "valor": 80.0}]
    Retorna o prontuário criado ou None se a consulta não estiver concluída.
    """
    ...
    for procedimento in procedimentos:
        valor_procedimentos = valor_procedimentos + procedimento["valor"]
    ...
    # Atualiza o valor total da consulta somando os procedimentos extras
    consulta["valor_total"] = consulta["valor_total"] + valor_procedimentos
    database.salvar(CONSULTAS_FILE, consultas)
...
def atualizar_prontuario(prontuario_id, novos_procedimentos, novas_observacoes):
    """
    Atualiza procedimentos e observações de um prontuário.
    Recalcula o valor dos procedimentos e atualiza a consulta vinculada.
    """
    for prontuario in prontuarios:
        if prontuario["id"] == prontuario_id:
            ...
            novo_valor = 0.0
            for procedimento in novos_procedimentos:
                novo_valor = novo_valor + procedimento["valor"]
            ...
            # Atualiza o valor total na consulta vinculada
...
def relatorio_consultas_por_periodo(data_inicio_str, data_fim_str):
    """
    Relatório 1: Lista todas as consultas de um período
    com total arrecadado no período.
    """
    ...
    for consulta in encontradas:
        if consulta["status"] == "concluida":
            total = total + consulta["valor_total"]

    return encontradas, total

def relatorio_consultas_por_medico(medico_id):
    """
    Relatório 2: Lista todas as consultas de um médico
    com contagem por status.
    """
...
def relatorio_financeiro_paciente(paciente_id):
    """
    Relatório 3 (diferencial): Total gasto por um paciente,
    detalhando consultas concluídas e multas por falta.
    """
    ...
    for consulta in consultas_paciente:
        if consulta["status"] == "concluida":
            total_consultas = total_consultas + consulta["valor_total"]
        elif consulta["status"] == "falta":
            total_multas = total_multas + MULTA_FALTA
